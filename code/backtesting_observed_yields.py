...
def process_forecast_outer(args):
    ...
    """
    try:
        maturity, series, execution_date, forecast_horizon, obs_yields_model_dir = args
        forecast_results = []
        ...
        maturity_dir = os.path.join(obs_yields_model_dir, "simulations", f"{maturity.replace(' ', '_').replace('years', 'maturity')}")
...
def run_yield_forecasts_parallel(country, observed_yields_df, obs_yields_model_dir, forecast_horizon=60, num_outer_workers=4):
    """
    ...
    for maturity in observed_yields_df.columns:
        ...
        for execution_date in series.index:
            ...
            tasks.append((maturity, series, execution_date, forecast_horizon, obs_yields_model_dir))
...
if __name__ == "__main__":
    ...
    obs_yields_dir = fr"{save_dir}\{country}\yields\observed_yields"
    os.makedirs(obs_yields_dir, exist_ok=True)

    obs_yields_model_dir = fr"{obs_yields_dir}\AR_1"
    os.makedirs(obs_yields_model_dir, exist_ok=True)
    ...
    df_predictions, df_insample_residuals, df_insample_metrics = run_yield_forecasts_parallel(
        country, 
        observed_yields_df_resampled,
        obs_yields_model_dir,
        forecast_horizon=60,
        num_outer_workers=4  
    )
    
    if df_predictions is not None:
            predictions_file = os.path.join(obs_yields_model_dir, f"forecasts.csv")
            df_predictions.to_csv(predictions_file, index=False)
            
            # Save residuals
            residuals_file = os.path.join(obs_yields_model_dir, f"residuals.csv")
            df_insample_residuals.to_csv(residuals_file, index=False)

            # Save in sample metrics
            insample_metrics_file = os.path.join(obs_yields_model_dir, f"insample_metrics.csv")
            ...
            metrics_by_horizon_file = os.path.join(obs_yields_model_dir, f"outofsample_metrics_by_horizon.csv")
            pd.concat(outofsample_metrics_by_horizon).to_csv(metrics_by_horizon_file, index=False)

            metrics_by_execution_date_file = os.path.join(obs_yields_model_dir, f"outofsample_metrics_by_execution_date.csv")
            pd.concat(outofsample_metrics_by_exec_date).to_csv(metrics_by_execution_date_file, index=False)

            metrics_by_row_file = os.path.join(obs_yields_model_dir, f"outofsample_metrics_by_row.csv")
